...
set domain to "http://illinois.edu"
set csv_path to "/Volumes/Documents/Users/tnip/Desktop/senior-projects-students-trim.csv"
...
repeat with student_record in l
	tell application "Google Chrome"
		open location domain & "/confluence/admin/users/createuser.action"
		tell active tab of window 1
			repeat while loading is true
				delay 0.1
			end repeat
			execute javascript "document.getElementById('username').value='" & item 1 in student_record & "';
                                document.getElementById('fullname').value='" & item 5 in student_record & " " & item 4 in student_record & "';
                                document.getElementById('password').value='" & item 2 in student_record & "';
                                document.getElementById('confirm').value='" & item 2 in student_record & "';
                                document.getElementById('email').value='" & item 1 in student_record & "@illinois.edu';
                                document.getElementById('sendEmail').checked = false;
                                document.getElementsByName('createuserform')[0].submit();"
		end tell
	end tell
end repeat
